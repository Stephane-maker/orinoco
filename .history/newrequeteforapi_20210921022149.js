...
function clickUserVoirMoins(idMoins, article, btnGeneral, idClassic) {
    let anotherFocusBtn = document.getElementById(idMoins);
    anotherFocusBtn.addEventListener("click", function() {
        for (let i = 0; i < 3; i++) {
            let t = document.getElementById(article);
            t.remove();
        }
        anotherFocusBtn.remove();
        const z = document.createElement("button");
        let b = document.getElementById(btnGeneral);
        b.appendChild(z);
...
function atclick(idMoins, article, btnGeneral, idClassic) {
    clickUserVoirMoins("buttonoursonVoirMoins", "containerArticle", "oursonButton", "buttonourson")
    clickUserVoirMoins("buttoncameraVoirMoins", "containerArticle", "cameraButton", "buttoncamera")
    clickUserVoirMoins("buttontableVoirMoins", "containerArticle", "tableButton", "buttontable")
}