...
class GeneralClassAccesoire {
    ...
    addLinkForGeneralClassAccesoire(re()) {
        let eltGeneral = document.getElementById(re());
        let detail = "";
        switch (this.type) {
            case "cameras":
...
function requete(elementRequete, valueLength) {
    ...
    fetch("http://localhost:3000/api/" + elementRequete)
        ...
        .then(function(value) {
            for (let i = 0; i < value.length; i++) {
                ...
                const newGeneralCLass = new GeneralClassAccesoire(tableauProduit);
                newGeneralCLass.addLinkForGeneralClassAccesoire(re());
            }
